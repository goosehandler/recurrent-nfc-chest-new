...
def dataloaders(dirname, array_type, num_measurements, t_SNR_range, v_SNR_range, train_batch_size, test_batch_size=None):
    ...
    if t_SNR_range == '10to20dB':
        SNR_range = [10, 20]
    elif t_SNR_range == '0to10dB':
        SNR_range = [0, 10]
    ...
    train_dataset = HybridFieldChannelDataset(**args_dict)
    test_dataset = HybridFieldChannelDataset(**args_dict)

    train_dataset.update_dataset(0)
    test_dataset.update_dataset(1000000)

    train_dataset.channels = train_dataset.channels.reshape((-1, 2048))
    train_dataset.measurements = train_dataset.measurements.reshape((-1, 1024))

    test_dataset.channels = test_dataset.channels.reshape((-1, 2048))
    test_dataset.measurements = test_dataset.measurements.reshape((-1, 1024))
...
def get_stats(net, test_loader, criterion):
    ...
    with torch.no_grad():
        for channels, measurements in test_loader:
            channels = channels.to(net.device())
            measurements = measurements.to(net.device())
            batch_size = measurements.shape[0]

            h_predict = net(measurements)
            batch_loss = criterion(h_predict, channels)
            test_loss += batch_size * batch_loss

    test_loss /= len(test_loader.dataset)
    test_NMSE = compute_NMSE(h_predict, channels)
...
def train_FPN_OAMP(net, max_epochs, lr_scheduler, train_loader,
    ...
    for epoch in range(max_epochs):
        ...
        # train_loader.dataset.update_dataset(epoch)
        ...
        with tqdm(total=tot, unit=" batch", leave=False, ascii=True) as tepoch:
            ...
            for _, (channels, measurements) in enumerate(train_loader):
                channels = channels.to(net.device())
                measurements = measurements.to(net.device())
                batch_size = measurements.shape[0]
        ...
        # validation_loader.dataset.update_dataset(epoch+max_epochs)

        validation_loss, validation_NMSE = get_stats(net,
                                         validation_loader,
                                         criterion)
        # validation_loader.dataset.delete_dataset()
        ...
        if validation_NMSE < best_validation_NMSE:
            ...
            file_name = save_dir + net.name() + '_weights_0to10dB.pth'
        ...
        if epoch+1 == max_epochs:
            ...
            file_name = save_dir + net.name() + '_history1.pth'
            torch.save(state, file_name)
